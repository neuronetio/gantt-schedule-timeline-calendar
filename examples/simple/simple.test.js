...
describe('Basic', () => {
  beforeAll(async () => {
    await page.goto(`http://localhost:${port}/examples/simple/simple.esm.html`);
  });

  it('should display things', async () => {
    // await expect(page).toMatchElement('.gstc__list-column-row-content', {
    //   text: 'Row 1',
    // });
  ...
  //   expect(topRow1).toEqual('gstcid-1');
  ...
  //     gstc.api.scrollToTime(gstc.api.time.date('2020-01-01').add(7, 'day').valueOf(), false);
  //     return state.get('$data.chart.time.leftGlobal');
  //   });
  ...
  //   await page.evaluate(() => {
  //     const item = state.get('config.chart.items.gstcid-1');
  //     gstc.api.setScrollTop(0);
  //     gstc.api.scrollToTime(gstc.api.time.date(item.time.start).valueOf(), false);
  //   });
  //   await expect(page).toMatchElement('.gstc__chart-timeline-items-row-item-label', {
  //     text: 'item id 1',
  //   });
  // });
  ...
  //       .querySelector('.gstc__list-column-header-resizer-dots--gstcid-label')
  //       .getBoundingClientRect();
  //     return { left, top };
  //   });
  //   const width1 = await page.evaluate(() => state.get('$data.list.columns.data.label.width'));
  //   await page.mouse.move(left + 2, top + 5);
  //   await page.mouse.down();
  //   await page.mouse.move(left + 102, top + 5);
  //   await page.mouse.up();
  //   const width2 = await page.evaluate(() => state.get('$data.list.columns.data.label.width'));
