 ...
 * http://polymer.github.io/PATENTS.txt
 */
(window.litHtmlVersions||(window.litHtmlVersions=[])).push("1.1.2");const E=(t,...e)=>new TemplateResult(t,e,"html",P),O=(t,...e)=>new SVGTemplateResult(t,e,"svg",P);
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 ...
 * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */function ChartTimeline(t,e){const{api:n,state:i,onDestroy:s,Actions:o,update:r,html:a,createComponent:l,StyleMap:c}=t,u=n.getActions("chart-timeline"),h=Object.assign(Object.assign({},e),{api:n,state:i});let d;s(i.subscribe("config.wrappers.ChartTimeline",t=>d=t));const p=i.get("config.components.ChartTimelineGrid"),f=i.get("config.components.ChartTimelineItems"),m=l(p);s(m.destroy);const g=l(f);let v,y;s(g.destroy),s(i.subscribe("config.classNames",()=>{v=n.getClass("chart-timeline"),y=n.getClass("chart-timeline-inner"),r()}));let b=new c({}),w=new c({});s(i.subscribeAll(["_internal.height","_internal.chart.dimensions.width","_internal.list.rowsHeight","config.scroll.compensation","_internal.chart.time.dates.day"],(function calculateStyle(){const t=n.getCompensationX(),e=n.getCompensationY(),s=i.get("_internal.chart.dimensions.width"),o=i.get("_internal.list.rowsHeight");b.style.height=i.get("_internal.height")+"px",b.style.width=s?s+"px":"0px",w.style.height=o+"px",w.style.width=s?s+t+"px":"0px",w.style.transform=`translate(-${t}px, ${e}px)`,r()}))),u.push(t=>{i.update("_internal.elements.chart-timeline",t)});const _=o.create(u,h);return i=>d(a`
 ...
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */
function ChartTimelineItems(t,e={}){const{api:n,state:i,onDestroy:s,Actions:o,update:r,html:a,reuseComponents:l,StyleMap:c}=t,u=n.getActions("chart-timeline-items");let h;s(i.subscribe("config.wrappers.ChartTimelineItems",t=>h=t));const d=i.get("config.components.ChartTimelineItemsRow");let p;s(i.subscribe("config.classNames",()=>{p=n.getClass("chart-timeline-items"),r()}));let f=new c({},!0);s(i.subscribeAll(["_internal.height","_internal.chart.dimensions.width","config.scroll.compensation","_internal.chart.time.dates.day"],()=>{const t=i.get("_internal.chart.dimensions.width"),e=i.get("_internal.height"),s=n.getCompensationY(),o=n.getCompensationX();f.style.width=t+o+"px",f.style.height=e+Math.abs(s)+"px"}));let m=[];s(i.subscribeAll(["_internal.list.visibleRows","config.chart.items","config.list.rows"],()=>{const t=i.get("_internal.list.visibleRows");m=l(m,t,t=>({row:t}),d),r()},{bulk:!0})),s((function destroyRows(){m.forEach(t=>t.destroy())}));const g=o.create(u,{api:n,state:i});return n=>h(a`
