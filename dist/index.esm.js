 ...
 * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */function Ut(t,e){const{api:n,state:s,onDestroy:i,actions:o,update:r,onChange:a,html:l}=t,c="chart-calendar-date",d=n.getActions(c);let u,h,p,f,m,g,v;i(s.subscribe("config.wrappers.ChartCalendarDate",t=>u=t));const b=n.time.date().format("YYYY-MM-DD");let y,w,_,$,x,C,M,P,T,A,O=s.subscribe("config.classNames",()=>{h=n.getClass(c,e),n.time.date(e.date.leftGlobal).format("YYYY-MM-DD")===n.time.date().format("YYYY-MM-DD")&&(h+=" current"),n.time.date(e.date.leftGlobal).subtract(1,"day").format("YYYY-MM-DD")===b&&(h+=" next"),n.time.date(e.date.leftGlobal).add(1,"day").format("YYYY-MM-DD")===b&&(h+=" previous"),p=n.getClass(`${c}-formatted`,e),f=n.getClass(`${c}-formatted-year`,e),m=n.getClass(`${c}-formatted-month`,e),g=n.getClass(`${c}-formatted-day`,e),v=n.getClass(`${c}-formatted-day-word`,e),r()});return a(t=>{e=t,A=s.subscribeAll(["_internal.chart.time","config.chart.calendar.vertical.smallFormat"],(function(){A&&A(),y=s.get("_internal.chart.time"),T=y.zoom<=22?18:13;const t=n.time.date(e.date.leftGlobal),i=y.maxWidth[y.period];w=i<=40;const o=s.get("config.chart.calendar.vertical.smallFormat");_=t.format(o),$=t.format("YYYY"),x=t.format("MMMM"),C=t.format("DD"),M=t.format("dddd"),i<=70?($=t.format("YY"),x=t.format("MMM"),C=t.format("DD"),M=t.format("ddd")):i<=150&&(M=t.format("ddd")),P=`width: ${e.date.width}px; margin-left:-${e.date.subPx}px; --day-size: ${T}px`,r()}),{bulk:!0})}),i(()=>{O(),A()}),i=>u(l`
        <div
          class=${h}
          style=${P}
