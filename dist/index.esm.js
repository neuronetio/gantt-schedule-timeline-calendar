...
function List(vido) {
    ...
    onDestroy(state.subscribe('config.list.columns.data;', data => {
        componentsFromDataArray(listColumns, Object.values(data), column => ({ columnId: column.id }), ListColumnComponent);
        update();
    }));
    onDestroy(() => {
...
function ListColumnRow(vido, { rowId, columnId }) {
    ...
    let ListExpanderComponent;
    onDestroy(state.subscribe('config.components.ListExpander', value => (ListExpanderComponent = value)));
    let row, rowPath = `_internal.flatTreeMapById.${rowId}`;
    let column, colPath = `config.list.columns.data.${columnId}`;
    let style;
    let rowSub, colSub;
    let ListExpander;
 ...
 * @license   GPL-3.0
 */
function ChartTimelineGridBlock(vido, { row, time, top }) {
    ...
    onChange(({ row, time, top }) => {
        ...
        if (typeof row.rowData.style === 'object' && row.rowData.style.constructor.name === 'Object') {
            if (typeof row.rowData.style.gridBlock === 'object' &&
 ...
 * @license   GPL-3.0
 */
function ChartTimelineGridRow(vido, { row }) {
