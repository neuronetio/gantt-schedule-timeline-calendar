 ...
 * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */function ListColumnRow(t,e){const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a,createComponent:l,onChange:c,styleMap:d}=t;let h,u;s(i.subscribe("config.wrappers.ListColumnRow",t=>h=t)),s(i.subscribe("config.components.ListExpander",t=>u=t));let p,f,m=`_internal.flatTreeMapById.${e.rowId}`,g=i.get(m),v=`config.list.columns.data.${e.columnId}`,b=i.get(v),y={width:"",height:"",visibility:"","--height":""};const w=l(u,{row:g});c((t,n)=>{if(n.leave)return y.visibility="hidden",void r();const s=(e=t).rowId,o=e.columnId;p&&p(),f&&f(),m=`_internal.flatTreeMapById.${s}`,v=`config.list.columns.data.${o}`,p=i.subscribe(m,t=>{g=t,y["--height"]=g.height+"px",y.width=e.width+"px",y.height=g.height+"px";for(let t of g._internal.parents){const e=i.get(`_internal.flatTreeMapById.${t}`);"object"==typeof e.style&&"Object"===e.style.constructor.name&&"string"==typeof e.style.children&&(y+=e.style.children)}"object"==typeof g.style&&"Object"===g.style.constructor.name&&"string"==typeof g.style.current&&(y+=g.style.current),r()}),w&&w.change({row:g}),f=i.subscribe(v,t=>{b=t,r()})}),s(()=>{w&&w.destroy(),f(),p()});const _=n.getActions("list-column-row");let $;return s(i.subscribe("config.classNames",t=>{$=n.getClass("list-column-row"),r()})),function updateTemplate(s){return h(a`
        <div
          class=${$}
          style=${d(y)}
 ...
 * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */const wt=(t,e)=>(e.state.update("_internal.elements.chart-timeline-items-rows",e=>(void 0===e&&(e=[]),e.push(t),e),{only:null}),{update(){},destroy(t){e.state.update("_internal.elements.chart-timeline-items-rows",e=>e.filter(e=>e!==t))}}),_t=(t,e)=>{const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a,onChange:l,reuseComponents:c,styleMap:d}=t;let h;s(i.subscribe("config.wrappers.ChartTimelineItemsRow",t=>h=t));const u=i.get("config.components.ChartTimelineItemsRowItem");let p,f,m=`_internal.flatTreeMapById.${e.row.id}._internal.items`,g={visibility:"",width:"",height:"",overflow:"hidden"},v={width:"",height:"",overflow:"hidden"},b=[];function updateDom(){if(!e)return void(g.visibility="hidden");const t=i.get("_internal.chart");g.visibility="visible",g.width=t.dimensions.width+"px",g.height=e.row.height+"px",g["--row-height"]=e.row.height+"px",v.width=t.time.totalViewDurationPx+"px",v.height=e.row.height+"px"}l((t,n)=>{if(n.leave)return g.visibility="hidden",r();((t,e)=>{if(e.leave)return updateDom(),r();m=`_internal.flatTreeMapById.${t.id}._internal.items`,"function"==typeof p&&p(),"function"==typeof f&&f(),p=i.subscribe("_internal.chart",(t,e)=>{updateDom(),r()}),f=i.subscribe(m,e=>{b=c(b,e,e=>({row:t,item:e}),u),updateDom(),r()})})((e=t).row,n)}),s(()=>{f(),p(),b.forEach(t=>t.destroy())});const y=n.getActions("chart-timeline-items-row");let w,_;return s(i.subscribe("config.classNames",()=>{w=n.getClass("chart-timeline-items-row",e),_=n.getClass("chart-timeline-items-row-inner",e),r()})),y.includes(wt)||y.push(wt),s=>h(a`
        <div
          class=${w}
          data-actions=${o(y,Object.assign(Object.assign({},e),{api:n,state:i}))}
          style=${d(g)}
        >
          <div class=${_} style=${d(v)}>
            ${b.map(t=>t.html())}
          </div>
        </div>
      `,{props:e,vido:t,templateProps:s})},$t=(t,e)=>(e.state.update("_internal.elements.chart-timeline-items-row-items",(function updateRowItems(e){return void 0===e&&(e=[]),e.push(t),e}),{only:null}),{update(){},destroy(t){e.state.update("_internal.elements.chart-timeline-items-row-items",e=>e.filter(e=>e!==t))}}),xt=(t,e)=>{const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a,onChange:l}=t;let c;s(i.subscribe("config.wrappers.ChartTimelineItemsRowItem",t=>c=t));let d,h,u=0,p=0;const f=t=>{h="";let n=i.get("_internal.chart.time");u=(e.item.time.start-n.leftGlobal)/n.timePerPixel,p=(e.item.time.end-e.item.time.start)/n.timePerPixel,p-=i.get("config.chart.spacing")||0,d=`transform: translate(${u}px, 0px); width:${p}px; `,"object"==typeof e.item.style&&"Object"===e.item.style.constructor.name&&"string"==typeof e.item.style.current&&(h+=e.item.style.current),r()};l((t,n)=>{if(n.leave)return d+="visibility: hidden;",r();e=t,f()});const m="chart-timeline-items-row-item",g=n.getActions(m);let v,b,y;return s(i.subscribe("config.classNames",()=>{v=n.getClass(m,e),b=n.getClass(m+"-content",e),y=n.getClass(m+"-content-label",e),r()})),s(i.subscribe("_internal.chart.time",t=>{f()})),-1===g.indexOf($t)&&g.push($t),function updateTemplate(s){return c(a`
        <div
          class=${v}
          data-actions=${o(g,{item:e.item,row:e.row,left:u,width:p,api:n,state:i})}
          style=${d}
        >
          <div class=${b} style=${h}>
