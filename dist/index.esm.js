 ...
 * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */function ChartTimelineGrid(t,e){const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a,reuseComponents:l,StyleMap:c}=t,d=n.getActions("chart-timeline-grid"),h={api:n,state:i};let u;s(i.subscribe("config.wrappers.ChartTimelineGrid",t=>u=t));const p=i.get("config.components.ChartTimelineGridRow");let f,m,g;s(i.subscribe("config.classNames",()=>{f=n.getClass("chart-timeline-grid"),r()})),s(i.subscribe("config.chart.time.period",t=>m=t)),s(i.subscribe("config.chart.grid.block.onCreate",t=>g=t));let v=[];const y=[],b=new Map,w=new c({});s(i.subscribeAll(["_internal.list.visibleRows;",`_internal.chart.time.dates.${m};`,"_internal.height","_internal.chart.dimensions.width"],()=>{const t=i.get("_internal.chart.dimensions.width"),e=i.get("_internal.height"),s=i.get(`_internal.chart.time.dates.${m}`),o=i.get("_internal.list.visibleRows");if(!s||0===s.length)return;const r=s[0].subPx;w.style.height=e+"px",w.style.width=t+"px",w.style.transform=`translate(-${r}px, 0px)`;let a=0;y.length=0;for(const e of o){const i=[];for(const t of s){let s;b.has(t.leftGlobal)?s=b.get(t.leftGlobal):(s=n.time.date(t.leftGlobal).format("YYYY-MM-DD"),b.set(t.leftGlobal,s));let o={id:e.id+":"+s,time:t,row:e,top:a};for(const t of g)o=t(o);i.push(o)}y.push({row:e,blocks:i,top:a,width:t}),a+=e.height}i.update("_internal.chart.grid.rowsWithBlocks",y)},{bulk:!0}));return s(i.subscribe("_internal.chart.grid.rowsWithBlocks",t=>{t&&(l(v,t,t=>t,p),r())})),d.includes((function bindElement(t){i.update("_internal.elements.chart-timeline-grid",t)}))||d.push(),s(()=>{v.forEach(t=>t.destroy())}),n=>u(a`
