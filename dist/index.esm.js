 ...
 * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */function Qt(t,e){return e.state.update("_internal.elements.chart-timeline-grid-row-blocks",e=>(void 0===e&&(e=[]),e.push(t),e),{only:null}),{destroy(t){e.state.update("_internal.elements.chart-timeline-grid-row-blocks",e=>e.filter(e=>e!==t))}}}function te(t,e){const{api:n,state:s,onDestroy:i,actions:o,update:r,html:a,onChange:l}=t,c="chart-timeline-grid-row-block",d=n.getActions(c);let u;i(s.subscribe("config.wrappers.ChartTimelineGridRowBlock",t=>{u=t,r()}));const h=n.time.date().startOf("day").valueOf();let p;function f(t){p=n.getClass(c),t.leftGlobal===h&&(p+=" current")}f(e.time);let m=`width: ${e.time.width}px;height: 100%;margin-left:-${e.time.subPx}px;`;return l(t=>{f((e=t).time),m=`width: ${e.time.width}px; height: 100%; margin-left:-${e.time.subPx}px; `;const n=s.get("config.list.rows");for(const t of e.row._internal.parents){const e=n[t];"object"==typeof e.style&&"object"==typeof e.style.gridBlock&&"string"==typeof e.style.gridBlock.children&&(m+=e.style.gridBlock.children)}"object"==typeof e.row.style&&"object"==typeof e.row.style.gridBlock&&"string"==typeof e.row.style.gridBlock.current&&(m+=e.row.style.gridBlock.current),r()}),-1===d.indexOf(Qt)&&d.push(Qt),()=>u(a`
