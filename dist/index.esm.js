 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */const b=` ${l} `;class w{constructor(t,e,n,i){this.strings=t,this.values=e,this.type=n,this.processor=i}getHTML(){const t=this.strings.length-1;let e="",n=!1;for(let i=0;i<t;i++){const t=this.strings[i],s=t.lastIndexOf("\x3c!--");n=(s>-1||n)&&-1===t.indexOf("--\x3e",s+1);const o=g.exec(t);e+=null===o?t+(n?b:c):t.substr(0,o.index)+o[1]+o[2]+h+o[3]+l}return e+=this.strings[t]}getTemplateElement(){const t=document.createElement("template");return t.innerHTML=this.getHTML(),t}}class y extends w{getHTML(){return`<svg>${super.getHTML()}</svg>`}getTemplateElement(){const t=super.getTemplateElement(),e=t.content,n=e.firstChild;return e.removeChild(n),s(e,n.firstChild),t}}
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */const O=new class{handleAttributeExpressions(t,e,n,i){const s=e[0];if("."===s){return new P(t,e.slice(1),n).parts}return"@"===s?[new I(t,e.slice(1),i.eventContext)]:"?"===s?[new T(t,e.slice(1),n)]:new x(t,e,n).parts}handleTextExpression(t){return new C(t)}};
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 ...
 * @package   gantt-schedule-timeline-calendar
 * @license   GPL-3.0
 */,ChartCalendarDate:function({date:t},e){const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a}=e,l="chart-calendar-date",c=n.getActions(l);let d,h,u,p,f,m,g,v,b,w,y,_,$,x,M,C;return s(i.subscribe("config.wrappers.ChartCalendarDate",t=>d=t)),s(i.subscribe("config.classNames",()=>{h=n.getClass(l,{date:t}),n.time.date(t.leftGlobal).format("YYYY-MM-DD")===n.time.date().format("YYYY-MM-DD")&&(h+=" current"),n.time.date(t.leftGlobal).subtract(1,"day").format("YYYY-MM-DD")===n.time.date().format("YYYY-MM-DD")&&(h+=" next"),n.time.date(t.leftGlobal).add(1,"day").format("YYYY-MM-DD")===n.time.date().format("YYYY-MM-DD")&&(h+=" previous"),u=n.getClass(`${l}-formatted`,{date:t}),p=n.getClass(`${l}-formatted-year`,{date:t}),f=n.getClass(`${l}-formatted-month`,{date:t}),m=n.getClass(`${l}-formatted-day`,{date:t}),g=n.getClass(`${l}-formatted-day-word`,{date:t}),r()})),s(i.subscribeAll(["_internal.chart.time","config.chart.calendar.vertical.smallFormat"],(function(){v=i.get("_internal.chart.time"),C=v.zoom<=22?18:13;const e=n.time.date(t.leftGlobal),s=v.maxWidth;b=s<=40;const o=i.get("config.chart.calendar.vertical.smallFormat");w=e.format(o),y=e.format("YYYY"),_=e.format("MMMM"),$=e.format("DD"),x=e.format("dddd"),s<=70?(y=e.format("YY"),_=e.format("MMM"),$=e.format("DD"),x=e.format("ddd")):s<=150&&(x=e.format("ddd")),M=`width: ${t.width}px; margin-left:-${t.subPx}px; --day-size: ${C}px`,r()}),{bulk:!0})),s=>d(a`
 ...
 * @package   gantt-schedule-timeline-calendar
 * @license   GPL-3.0
 */,ChartTimelineGrid:function(t){const{api:e,state:n,onDestroy:i,actions:s,update:o,html:r,createComponent:a,repeat:l}=t,c=e.getActions("chart-timeline-grid");let d;i(n.subscribe("config.wrappers.ChartTimelineGrid",t=>d=t));const h=n.get("config.components.ChartTimelineGridRow");let u,p,f;i(n.subscribe("config.classNames",()=>{u=e.getClass("chart-timeline-grid"),o()})),i(n.subscribe("_internal.height",t=>{f=`height: ${p=t}px`,o()}));let m,g=[];return i(n.subscribeAll(["_internal.chart.time.dates","_internal.list.visibleRows","config.chart.grid.block"],(function(){const t=n.get("_internal.list.visibleRows"),e=n.get("_internal.chart.time.dates");g.forEach(t=>t.component.destroy()),g=[];let i=0;m=[];for(const n in t){const s=t[n],r=[];let l=0;for(const t of e)r.push({id:l++,date:t,row:s,top:i});const c={id:s.id,blocks:r,rowData:s,top:i};m.push(c),g.push({id:s.id,component:a(h,{row:c})}),i+=s.height,o()}}),{bulk:!0})),c.push(t=>{n.update("_internal.elements.grid")}),i(()=>{g.forEach(t=>t.component.destroy())}),i=>d(r`
 ...
 * @package   gantt-schedule-timeline-calendar
 * @license   GPL-3.0
 */,ChartTimelineItemsRowItem:function({rowId:t,itemId:e},n){const{api:i,state:s,onDestroy:o,actions:r,update:a,html:l}=n;let c;o(s.subscribe("config.wrappers.ChartTimelineItemsRowItem",t=>c=t));let d,h=`config.list.rows.${t}`;o(s.subscribe(h,t=>{d=t,a()}));let u,p=`config.chart.items.${e}`;o(s.subscribe(p,t=>{u=t,a()}));const f="chart-timeline-items-row-item",m=i.getActions(f);let g,v,b;o(s.subscribe("config.classNames",()=>{g=i.getClass(f,{row:d,item:u}),v=i.getClass(f+"-content",{row:d,item:u}),b=i.getClass(f+"-content-label",{row:d,item:u}),a()}));let w,y=0,_=0;return o(s.subscribeAll(["_internal.chart.time","config.scroll",p],t=>{u=s.get(p);let e=s.get("_internal.chart.time");y=(u.time.start-e.leftGlobal)/e.timePerPixel,_=(u.time.end-u.time.start)/e.timePerPixel,_-=s.get("config.chart.spacing");i.isItemInViewport(u,e.leftGlobal,e.rightGlobal);w=`left:${y}px;width:${_}px;`,a()},{bulk:!0})),o=>c(l`
    <div
      class=${g}
      data-actions=${r(m,{item:u,row:d,left:y,width:_,api:i,state:s})}
      style=${w}
    >
      <div class=${v}>
