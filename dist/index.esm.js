 ...
 * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */function ChartTimelineGrid(t,e){const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a,reuseComponents:l}=t,c=n.getActions("chart-timeline-grid");let d;s(i.subscribe("config.wrappers.ChartTimelineGrid",t=>d=t));const u=i.get("config.components.ChartTimelineGridRow");let h,p,f;s(i.subscribe("config.classNames",()=>{h=n.getClass("chart-timeline-grid"),r()})),s(i.subscribe("config.chart.time.period",t=>p=t)),s(i.subscribe("config.chart.grid.block.onCreate",t=>f=t));let m=[];const g=[],v=new Map;let b;s(i.subscribeAll(["_internal.list.visibleRows;",`_internal.chart.time.dates.${p};`,"_internal.height","_internal.chart.dimensions.width"],()=>{const t=i.get("_internal.chart.dimensions.width"),e=i.get("_internal.height"),s=i.get(`_internal.chart.time.dates.${p}`),o=i.get("_internal.list.visibleRows");if(!s||0===s.length)return;const r=s[0].subPx;b=`height: ${e}px; width: ${t}px; transform: translate(-${r}px, 0px);`;let a=0;g.length=0;for(const e of o){const i=[];for(const t of s){let s;v.has(t.leftGlobal)?s=v.get(t.leftGlobal):(s=n.time.date(t.leftGlobal).format("YYYY-MM-DD"),v.set(t.leftGlobal,s));let o={id:e.id+":"+s,time:t,row:e,top:a};for(const t of f)o=t(o);i.push(o)}g.push({row:e,blocks:i,top:a,width:t}),a+=e.height}i.update("_internal.chart.grid.rowsWithBlocks",g)},{bulk:!0}));return s(i.subscribe("_internal.chart.grid.rowsWithBlocks",t=>{t&&(l(m,t,t=>t,u),r())})),c.includes((function bindElement(t){i.update("_internal.elements.chart-timeline-grid",t)}))||c.push(),s(()=>{m.forEach(t=>t.destroy())}),s=>d(a`
        <div class=${h} data-actions=${o(c,{api:n,state:i})} style=${b}>
          ${m.map(t=>t.html())}
        </div>
      `,{props:e,vido:t,templateProps:s})}
/**
 ...
 * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */const bt=(t,e)=>(e.state.update("_internal.elements.chart-timeline-grid-row-blocks",e=>(void 0===e&&(e=[]),e.push(t),e),{only:null}),t=>{e.state.update("_internal.elements.chart-timeline-grid-row-blocks",e=>e.filter(e=>e!==t),{only:[""]})}),yt=(t,e)=>{const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a,onChange:l}=t,c=n.getActions("chart-timeline-grid-row-block");let d;s(i.subscribe("config.wrappers.ChartTimelineGridRowBlock",t=>{d=t,r()}));const u=n.time.date().startOf("day").valueOf();let h,p;const f=t=>{h=n.getClass("chart-timeline-grid-row-block"),p=h+"-content",t.leftGlobal===u&&(h+=" current")};f(e.time);let m=`width: ${e.time.width}px;height: ${e.row.height}px;`;return l((t,n)=>{if(n.leave)return m="visibility: hidden;",r();f((e=t).time),m=`width: ${e.time.width}px; height: ${e.row.height}px;`;const s=i.get("config.list.rows");for(const t of e.row._internal.parents){const e=s[t];"object"==typeof e.style&&"object"==typeof e.style.gridBlock&&"string"==typeof e.style.gridBlock.children&&(m+=e.style.gridBlock.children)}"object"==typeof e.row.style&&"object"==typeof e.row.style.gridBlock&&"string"==typeof e.row.style.gridBlock.current&&(m+=e.row.style.gridBlock.current),r()}),-1===c.indexOf(bt)&&c.push(bt),s=>d(a`
        <div class=${h} data-actions=${o(c,Object.assign(Object.assign({},e),{api:n,state:i}))} style=${m}>
          <div class=${p} />
        </div>
      `,{props:e,vido:t,templateProps:s})};
/**
 ...
 * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
 * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
 */const wt=(t,e)=>(e.state.update("_internal.elements.chart-timeline-items-rows",e=>(void 0===e&&(e=[]),e.push(t),e),{only:null}),{update(){},destroy(t){e.state.update("_internal.elements.chart-timeline-items-rows",e=>e.filter(e=>e!==t))}}),_t=(t,e)=>{const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a,onChange:l,reuseComponents:c}=t;let d;s(i.subscribe("config.wrappers.ChartTimelineItemsRow",t=>d=t));const u=i.get("config.components.ChartTimelineItemsRowItem");let h,p,f,m,g=`_internal.flatTreeMapById.${e.row.id}._internal.items`,v=[];function updateDom(){if(!e)return void(f="visibility: hidden;");const t=i.get("_internal.chart");f=`width:${t.dimensions.width}px; height:${e.row.height}px; --row-height:${e.row.height}px;`,m=`width: ${t.time.totalViewDurationPx}px; height: ${e.row.height}px;`}l((t,n)=>{if(n.leave)return f="visibility: hidden;",r();((t,e)=>{if(e.leave)return updateDom(),r();g=`_internal.flatTreeMapById.${t.id}._internal.items`,"function"==typeof h&&h(),"function"==typeof p&&p(),h=i.subscribe("_internal.chart",(t,e)=>{updateDom(),r()}),p=i.subscribe(g,e=>{v=c(v,e,e=>({row:t,item:e}),u),updateDom(),r()})})((e=t).row,n)}),s(()=>{p(),h(),v.forEach(t=>t.destroy())});const b=n.getActions("chart-timeline-items-row");let y,w;return s(i.subscribe("config.classNames",()=>{y=n.getClass("chart-timeline-items-row",e),w=n.getClass("chart-timeline-items-row-inner",e),r()})),b.includes(wt)||b.push(wt),s=>d(a`
        <div class=${y} data-actions=${o(b,Object.assign(Object.assign({},e),{api:n,state:i}))} style=${f}>
          <div class=${w} style=${m}>
            ${v.map(t=>t.html())}
          </div>
        </div>
      `,{props:e,vido:t,templateProps:s})},$t=(t,e)=>(e.state.update("_internal.elements.chart-timeline-items-row-items",(function updateRowItems(e){return void 0===e&&(e=[]),e.push(t),e}),{only:null}),{update(){},destroy(t){e.state.update("_internal.elements.chart-timeline-items-row-items",e=>e.filter(e=>e!==t))}}),xt=(t,e)=>{const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a,onChange:l}=t;let c;s(i.subscribe("config.wrappers.ChartTimelineItemsRowItem",t=>c=t));let d,u,h=0,p=0;const f=t=>{u="";let n=i.get("_internal.chart.time");h=(e.item.time.start-n.leftGlobal)/n.timePerPixel,p=(e.item.time.end-e.item.time.start)/n.timePerPixel,p-=i.get("config.chart.spacing")||0,d=`transform: translate(${h}px, 0px); width:${p}px; `,"object"==typeof e.item.style&&"Object"===e.item.style.constructor.name&&"string"==typeof e.item.style.current&&(u+=e.item.style.current),r()};l((t,n)=>{if(n.leave)return d+="visibility: hidden;",r();e=t,f()});const m="chart-timeline-items-row-item",g=n.getActions(m);let v,b,y;return s(i.subscribe("config.classNames",()=>{v=n.getClass(m,e),b=n.getClass(m+"-content",e),y=n.getClass(m+"-content-label",e),r()})),s(i.subscribe("_internal.chart.time",t=>{f()})),-1===g.indexOf($t)&&g.push($t),function updateTemplate(s){return c(a`
        <div
          class=${v}
          data-actions=${o(g,{item:e.item,row:e.row,left:h,width:p,api:n,state:i})}
          style=${d}
        >
          <div class=${b} style=${u}>
 ...
 * @license   GPL-3.0
 */
const It="gantt-schedule-timeline-calendar";function isObject$1(t){return t&&"object"==typeof t&&!Array.isArray(t)}function mergeDeep$1(t,...e){const n=e.shift();if(isObject$1(t)&&isObject$1(n))for(const e in n)if(isObject$1(n[e]))void 0===t[e]&&(t[e]={}),t[e]=mergeDeep$1(t[e],n[e]);else if(Array.isArray(n[e])){t[e]=[];for(let i of n[e])isObject$1(i)?t[e].push(mergeDeep$1({},i)):t[e].push(i)}else t[e]=n[e];return e.length?mergeDeep$1(t,...e):t}const Tt={name:It,stateFromConfig:function stateFromConfig(t){const e=defaultConfig(),n=function mergeActions(t,e){const n=mergeDeep$1({},e.actions),i=mergeDeep$1({},t.actions);let s=[...Object.keys(n),...Object.keys(i)];s=s.filter(t=>s.includes(t));const o={};for(const t of s)o[t]=[],void 0!==n[t]&&Array.isArray(n[t])&&(o[t]=[...n[t]]),void 0!==i[t]&&Array.isArray(i[t])&&(o[t]=[...o[t],...i[t]]);return delete t.actions,delete e.actions,o}(t,e),i={config:mergeDeep$1({},e,t)};return i.config.actions=n,new DeepState(i,{delimeter:"."})},mergeDeep:mergeDeep$1,date:t=>t?Mt(t):Mt(),dayjs:Mt};
/**
 * Gantt-Schedule-Timeline-Calendar
 *
 * @copyright Rafal Pospiech <https://neuronet.io>
