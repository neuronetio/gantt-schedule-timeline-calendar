 ...
 */
String(Math.random()).slice(2),document.createComment(""),window.trustedTypes&&trustedTypes.createPolicy("lit-html",{createHTML:t=>t}),document.createElement("template"),document.createTextNode("");let N=!1;(()=>{try{const t={get capture(){return N=!0,!1}};window.addEventListener("test",t,t),window.removeEventListener("test",t,t)}catch(t){}})(),
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 ...
 * http://polymer.github.io/PATENTS.txt
 */
document.createElement("template"),document.createTextNode("");function j(t,e=""){let n="gstc__"+t;return"gstc"===t&&(n="gstc"),e?`${n} ${n}--${e.replace(":","-")}`:n}const A="config.plugin.ProgressBar";class W{constructor(t,e){this.options=function(t){return Object.assign({enabled:!0,className:j("chart-timeline-items-row-item-progress-bar")},t)}(t),this.vido=e,this.state=e.state,this.className=this.options.className,this.slotComponent=this.slotComponent.bind(this),this.state.update("config.slots.chart-timeline-items-row-item.inner",t=>(t.includes(this.slotComponent)||t.push(this.slotComponent),t))}destroy(){this.state.update("config.slots.chart-timeline-items-row-item.inner",t=>t.filter(t=>t!==this.slotComponent))}slotComponent(t,e){let n;return t.onChange(r=>{if(!(e=r).item||void 0===e.item.progress)return void(n=0);const i=t.api.getItemData(e.item.id),s=i.position.right-i.position.actualRight;n=i.width*(e.item.progress/100)-s}),t=>void 0!==e.item.progress?this.vido.html`
        <div class="${this.className}" style=${`width: ${n}px;`}></div>${t}
      `:null}}function k(t={}){return function(e){const n=e.state.get("config.plugin.ProgressBar");n&&(t=e.api.mergeDeep({},t,n));return new W(t,e).destroy}}export{k as Plugin,A as pluginPath};
