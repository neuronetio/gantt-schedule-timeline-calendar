 ...
 */
String(Math.random()).slice(2),document.createComment(""),window.trustedTypes&&trustedTypes.createPolicy("lit-html",{createHTML:t=>t}),document.createElement("template"),document.createTextNode("");let T=!1;(()=>{try{const t={get capture(){return T=!0,!1}};window.addEventListener("test",t,t),window.removeEventListener("test",t,t)}catch(t){}})(),
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 ...
 * http://polymer.github.io/PATENTS.txt
 */
document.createElement("template"),document.createTextNode("");function j(t,e=""){let n="gstc__"+t;return"gstc"===t&&(n="gstc"),e?`${n} ${n}--${e.replace(":","-")}`:n}const k="config.plugin.TimeBookmarks",A="config.slots.chart.content";class W{constructor(t,e){this.unsub=[],this.options=function(t){return Object.assign({enabled:!0,className:j("chart-time-bookmark")},t)}(t),this.vido=e,this.state=e.state,this.className=this.options.className,this.slotComponent=this.slotComponent.bind(this),this.state.update(k,this.options),this.state.update(A,t=>(t.includes(this.slotComponent)||t.push(this.slotComponent),t)),this.unsub.push(this.state.subscribe(k,t=>this.options=t))}destroy(){this.unsub.forEach(t=>t()),this.state.update(A,t=>t.filter(t=>t!==this.slotComponent))}slotComponent(t){const{html:e,onDestroy:n,state:r,update:i,api:s}=t,o="#3498DB",a=[];n(r.subscribeAll([k,"config.chart.time","config.scroll.horizontal.dataIndex"],()=>{const t=r.get(k);a.length=0;for(const e in t.bookmarks){const n=Object.assign(Object.assign({},t.bookmarks[e]),{id:e,leftViewPx:0});n.id=e,n.leftViewPx=s.time.getViewOffsetPxFromDates(s.time.date(n.time)),a.push(n)}i()}));let u=0;n(r.subscribe("config.headerHeight",t=>{u=t,i()}));let l=0;n(r.subscribe("config.innerHeight",t=>{l=t,i()}));let h=0;n(r.subscribe("config.scroll.horizontal.size",t=>{h=t,i()}));const c=t=>e`<div
      ...
      >
        ...
        <div class="${this.className}s" style="height: ${l+u-h}px;">
          ${a.map(t=>c(t))}
        </div>`}}function H(t={}){return function(e){const n=e.state.get(k);n&&(t=e.api.mergeDeep({},t,n));return new W(t,e).destroy}}export{H as Plugin,k as pluginPath,A as slotPath};
