...
/**
     ...
     * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
     * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
     */const mt=(t,e)=>(e.state.update("_internal.elements.chart-timeline-grid-row-blocks",e=>(void 0===e&&(e=[]),e.push(t),e),{only:null}),t=>{e.state.update("_internal.elements.chart-timeline-grid-row-blocks",e=>e.filter(e=>e!==t),{only:[""]})}),gt=(t,e)=>{const{api:n,state:i,onDestroy:s,actions:o,update:r,html:a,onChange:l}=t,c=n.getActions("chart-timeline-grid-row-block");let d;s(i.subscribe("config.wrappers.ChartTimelineGridRowBlock",t=>{d=t,r()}));const h=n.time.date().startOf("day").valueOf();let u,p;const f=t=>{u=n.getClass("chart-timeline-grid-row-block"),p=u+"-content",t.leftGlobal===h&&(u+=" current")};f(e.time);let m=`width: ${e.time.width}px;height: ${e.row.height}px;`;return l((t,n)=>{if(n.leave)return m="visibility: hidden;",r();f((e=t).time),m=`width: ${e.time.width}px; height: ${e.row.height}px;`;const s=i.get("config.list.rows");for(const t of e.row._internal.parents){const e=s[t];"object"==typeof e.style&&"object"==typeof e.style.gridBlock&&"string"==typeof e.style.gridBlock.children&&(m+=e.style.gridBlock.children)}"object"==typeof e.row.style&&"object"==typeof e.row.style.gridBlock&&"string"==typeof e.row.style.gridBlock.current&&(m+=e.row.style.gridBlock.current),r()}),-1===c.indexOf(mt)&&c.push(mt),s=>d(a`
