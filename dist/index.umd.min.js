...
/**
     ...
     * @license   GPL-3.0 (https://github.com/neuronetio/gantt-schedule-timeline-calendar/blob/master/LICENSE)
     * @link      https://github.com/neuronetio/gantt-schedule-timeline-calendar
     */function ChartTimelineItems(t,e={}){const{api:n,state:i,onDestroy:s,Actions:o,update:r,html:a,reuseComponents:l,StyleMap:c}=t,d=n.getActions("chart-timeline-items");let h;s(i.subscribe("config.wrappers.ChartTimelineItems",t=>h=t));const u=i.get("config.components.ChartTimelineItemsRow");let p;s(i.subscribe("config.classNames",()=>{p=n.getClass("chart-timeline-items"),r()}));const m=new c({},!0);s(i.subscribeAll(["_internal.height","_internal.chart.dimensions.width","config.scroll.compensation","_internal.chart.time.dates.day"],(function calculateStyle(){const t=i.get("_internal.chart.dimensions.width"),e=i.get("_internal.height"),s=n.getCompensationY(),o=n.getCompensationX();m.style.width=t+o+"px",m.style.height=e+Math.abs(s)+"px"})));const f=[];s(i.subscribeAll(["_internal.list.visibleRows;","config.chart.items"],(function createRowComponents(){const t=i.get("_internal.list.visibleRows"),e=l(f,t||[],t=>({row:t}),u);return r(),e})));const g=o.create(d,{api:n,state:i});return n=>h(a`
